...
const UserRatings = () => {
  ...
  const [filteredRatings, setFilteredRatings] = useState<RatedContentItem[]>(ratings || []);
  const { data: session, status } = useSession();
  const filteredRatingsItems = filteredRatings.length > 0 ? filteredRatings : ratings || [];
  ...
  const handleGetRecommendations = async () => {
    try {
      ...
      console.log(recommendations); // Replace this with whatever you want to do with the recommendations
    } catch (error) {
    ...
    <div className="flex bg-gray-100 rounded-xl md:px-12 pt-12 gap-4 w-5/6">
      <div className="flex flex-col w-full">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold container mx-auto px-4">My Ratings</h1>
          <button
