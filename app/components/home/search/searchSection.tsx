...
const SearchSection = () => {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const response = await apiClient.get(`/api/Media_search?query=${encodeURIComponent(query)}`);
    ...
    <div className="text-white text-left p-8 pt-2 pb-4 flex-1 xl:max-w-4xl max-w-lg">
      ...
      <div className="mt-4">
        <div className="mt-4" style={{ height: searchHistory.length > 0 ? "auto" : "48px" }}>
          <div className={`flex flex-row gap-4 justify-between ${searchHistory.length === 0 ? "invisible" : "visible"}`}>
          ...
          <ul className={`flex flex-row flex-wrap gap-4 overflow-y-scroll ${searchHistory.length === 0 ? "invisible" : "visible"}`}>
